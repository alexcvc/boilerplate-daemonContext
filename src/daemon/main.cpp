...
static void show_version(const char* prog) {
  auto verString = version::daemon::getVersion(true);
  // info
...
handleConsoleType handle_console() {
  ...
  switch (key) {
    ...
    case 'v':
      // info
      std::cout << " v." << version::daemon::getVersion(true) << std::endl;
      break;
    case '?':
    case 'h':
...
void TaskAppContextFunc(app::DaemonConfig& daemon_config, std::stop_token token) {
  ...
  while (true) {
    ...
    if (token.stop_requested()) {
      spdlog::info("got stop requested in the application task");
      break;
    }
  }  // End of while loop
...
int main(int argc, char** argv) {
  ...
  if (daemonConfig.isDaemon) {
    if (auto result = daemon.make_daemon(daemonConfig.pidFile); result.has_value() && !result.value()) {
      spdlog::warn("Error starting the daemon.");
      return EXIT_FAILURE;
    }
  }
  ...
  taskAppContext = std::move(std::thread(TaskAppContextFunc, std::ref(daemonConfig), stop_src.get_token()));
  ...
  spdlog::info("The daemon process is stopping. Stop requested for an application task");
  ...
  spdlog::info("Main waits for completion of application task");
